        ...
        <div class="aside-wrapper">
            <aside id="no-ajax">
                <div class="title-wrapper stickable">
                    <div class="title">
                        <a href="{{ route('book', ['book' => $book]) }}" @class(['active' => !isset($poem)])>«{{ $book->title }}»</a>
                        <div class="filter">
                ...
                <ol>
                    @foreach ($book->poems as $bookPoem)
                        <li><a href="{{ route('poem', ['book' => $book, 'poem' => $bookPoem]) }}" data-dedication="{{ $bookPoem->dedication }}" @class(['active' => isset($poem) && ($poem->slug === $bookPoem->slug)])>{{ Str::of($bookPoem->title)->stripTags() }}</a></li>
                    @endforeach
                </ol>
            </aside>
