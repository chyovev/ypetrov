    ...
    <div class="aside-wrapper">
        <aside>
            <div class="title-wrapper stickable">
                <div class="title">
                    <a href="{url params=['controller' => 'books', 'action' => 'view', 'book' => $book['slug']]}"{if ! isset($poem)} class="active"{/if}>«{$book['title']}»</a>
                    <div class="filter">
            ...
            <ol>
                {foreach $book['contents'] as $poemSlug => $contentItem}
                    ...
                    <li><a href="{url params=['controller' => 'poems', 'action' => 'view', 'book' => $book['slug'], 'poem' => $poemSlug]}"{if isset($poem['slug']) && $poemSlug === $poem['slug']} class="active"{/if} title="{escape($item['title'])}" data-dedication="{escape($item['dedication'])}">{escape($item['title'])}</a></li>
                {/foreach}
            </ol>
        </aside>
