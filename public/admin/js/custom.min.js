...
let App = {
    ...
    ///////////////////////////////////////////////////////////////////////////
    /**
     * Initialize TinyMCE's WYSIWYG editor with the respective options.
     */
    initWysiwyg: function() {
        tinymce.init({
            ...
            language: 'bg_BG',
            language_url: '/admin/js/lib/tinymce/plugins/help/js/i18n/keynav/bg_BG.js',
            content_css : '/admin/css/tinymce.css',
            style_formats: [
                { title: 'Headings', items: [
                    { title: 'Default', block: 'h5' },
                ] },
                { title: 'Body', items: [
                    { title: 'Default', block: 'p' },
                    { title: 'Quote',   block: 'blockquote' },
                    { title: 'Poem',    block: 'div', classes: 'poem' },
                ] },
            ],
            image_class_list: [
                { title: 'Desktop', value: 'desktop-only' },
                { title: 'Mobile',  value: 'mobile-only' },
            ],
            menubar: false,
            plugins: "anchor autolink code image link lists media searchreplace table",
            toolbar1: "bold italic underline | styles | alignleft aligncenter alignright alignjustify | outdent indent | bullist numlist",
            toolbar2: "image link anchor | forecolor backcolor | subscript superscript | removeformat | hr | table | code",
            formats: {
    ...
    ///////////////////////////////////////////////////////////////////////////
    /**
     * Initialize Loe's jQuery multi-select plugin and combine it with
     * Deux Huit Huit's QuickSearch to allow for better UX.
     * 
     * NB! Keep in mind that after each select/deselect the QuickSearch cache
     *     should manually be refreshed to avoid already selected/deselected
     *     options from being shown again.
     */
    initMultiSelect: function() {
        $('.multi-select').multiSelect({
            keepOrder: true,
            selectableHeader: "<input type='text' class='form-control input-default mb-3 search-input' autocomplete='off' />",
            selectionHeader:  "<input type='text' class='form-control input-default mb-3 search-input' autocomplete='off' />",
            afterInit: function() {
                let $leftInput = this.$selectableUl.prev(),
                    leftTarget = '#' + this.$container.attr('id') + ' .ms-elem-selectable:not(.ms-selected)',

                    $rightInput = this.$selectionUl.prev(),
                    rightTarget = '#' + this.$container.attr('id') + ' .ms-elem-selection.ms-selected';

                this.$leftSearch  = $leftInput.quicksearch(leftTarget);
                this.$rightSearch = $rightInput.quicksearch(rightTarget);
            },
            afterSelect: function() {
                if (this.$leftSearch)  this.$leftSearch.cache();
                if (this.$rightSearch) this.$rightSearch.cache();
            },
